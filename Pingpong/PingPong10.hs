...
main = do
      ...
      setMaterial
      idleCallback $= Just (temporizador pelotaX pelotaY pelotaZ tiempo ida altura t fuerza preX)
...
desplegar x1 y1 z1 (xp, yp)= do 
               ...
               --preservingMatrix $ do--MESA
                 ...
                 dibPaleta (xu, yu, 0.95)--generar (0,-0.3,0) 1.8 0.2 2
...
temporizador pelotaX   pelotaY pelotaZ tiempo  ida   altura  ti fuerza preX= do
        z <- get pelotaZ
        y <- get pelotaY      
        t <- get tiempo
        id <- get ida
