...
main = do
      ...
      displayCallback $= desplegar pelotaX pelotaY pelotaZ (posX, posY) m puntos anguloPelota
      keyboardMouseCallback $= Just (click m rx)
      setMaterial
      idleCallback $= Just (temporizador pelotaX pelotaY pelotaZ tiempo ida altura t fuerza preX posX posY puntos rx m golpe anguloPelota)
...
desplegar x1 y1 z1 (xp, yp) pendiente puntos anguloPelota= do 
               --clear [ColorBuffer]
               x<- get x1
               y<- get y1
               z<- get z1
...
temporizador pelotaX   pelotaY pelotaZ tiempo  ida   altura  ti fuerza preX xleta yleta puntos rx pendiente golpe anguloPelota= do
        z <- get pelotaZ
        y <- get pelotaY 
        x <- get pelotaX     
        t <- get tiempo
                          ...
                          if ((z+ 0.1)<= (_DisPelota + 0.09) && (z+0.1)>= (_DisPelota - 0.09) && y<= (yu + _RADIO) &&  y>= (yu- _RADIO) && x<= (xu + _RADIO) &&  x>= (xu - _RADIO) ) then do
                                     ...
                                     if (angX==0) then
                                         pelotaX$=x
                                            ...
                                            let entre = ((m*x)-(m*xu))
                                            puntos$= m
                                            let xk = (xu*z+m*xu*x-m*xu*xu-_DisPelota*xu)/ (z-_DisPelota+entre)
                                            let zk = -m * (xk-x) +z
                                            
                  
                                            let co = sqrt(((xk-x)*(xk-x))+((zk-z)*(zk-z)))
                                            let h = sqrt(((xu-x)*(xu-x))+((_DisPelota-z)*(_DisPelota-z)))
       
                                            --let teta= asin (co/h)-----Angulo con el que llega la pelota
                                            rx $= atan((z-_DisPelota)/(x-xu))
                                            anguloPelota$= atan((z-_DisPelota)/(x-xu))
