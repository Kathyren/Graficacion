...
main = do
      ...
      displayCallback $= desplegar pelotaX pelotaY pelotaZ (posX, posY) m puntos anguloPelota estado modo
      keyboardMouseCallback $= Just (click m rx estado modo)

      --e <- get estado
      setMaterial
      
      idleCallback $= Just (temporizador pelotaX pelotaY pelotaZ tiempo ida altura t fuerza preX posX posY puntos rx m golpe anguloPelota tiempoLapso estado)
...
desplegar x1 y1 z1 (xp, yp) pendiente puntos anguloPelota estado modo= do 
               --clear [ColorBuffer]
               x<- get x1
               y<- get y1
               z<- get z1
               ...
               if (mo==0) then do 
                          ...
                          currentRasterPosition $= Vertex4 (-1) (-0.2) 0.0 1.0
                          ...
                          preservingMatrix $ do------Paleta y Mesa
                            ...
                            rotate 50 $ Vector3 0 1 (0::GLfloat)
                            dibPaleta ((xu+0.25), (yu+0.25), _DisPelota) mi
                            dibPaleta ((xu+0.25), (yu+0.25), (_DisPelota+0.1)) mi
                            generar (0,-0.3,0) 1.8 0.2 2
                          preservingMatrix $ do ---LINEAS PARA PERSPECIVA
...
dibBlanco (px, py, pz) r= do 
 ...
 -- pz<- get pZx
  ...
  translate $ Vector3 (px) (py) ((pz)::GLfloat)
  renderPrimitive Polygon $
      mapM_ (\(x, y, z) -> vertex $ Vertex3 x y z) (blanco r)
  translate $ Vector3 (-px) (-py) ((-pz)::GLfloat)
  flush

blanco :: GLfloat -> [(GLfloat,GLfloat,GLfloat)]
blanco r =  [ ((r*(sin (2*pi*k/20))), (r* (cos (2*pi*k/20))), ((r*(sin (2*pi*k/20))))) | k <- [1..20] ]
...
temporizador pelotaX   pelotaY pelotaZ tiempo  ida   altura  ti fuerza preX xleta yleta puntos rx pendiente golpe anguloPelota tl estado= do
       z <- get pelotaZ
       y <- get pelotaY 
       x <- get pelotaX     
       t <- get tiempo
