...
main = do
      ...
      displayCallback $= desplegar pelotaX pelotaY pelotaZ (posX, posY) m
      keyboardMouseCallback $= Just (click m)
      setMaterial
      idleCallback $= Just (temporizador pelotaX pelotaY pelotaZ tiempo ida altura t fuerza preX posX posY puntos)
...
desplegar x1 y1 z1 (xp, yp) pendiente= do 
               --clear [ColorBuffer]
               x<- get x1
               y<- get y1
               z<- get z1
...
temporizador pelotaX   pelotaY pelotaZ tiempo  ida   altura  ti fuerza preX xleta yleta puntos= do
        z <- get pelotaZ
        y <- get pelotaY 
        x <- get pelotaX     
        t <- get tiempo
...
click  m (MouseButton LeftButton) Down _ _ = do
        m$= -0.3
        postRedisplay Nothing
click  m (MouseButton RightButton) Down _ _ = do
        m$= 0.3
        postRedisplay Nothing
click  m _ Up _ _ = do
        m$= 0
