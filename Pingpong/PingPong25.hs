...
main = do
      ...
      idleCallback $=Just (temporizador pelotaX pelotaY pelotaZ tiempo ida altura t fuerza preX posX posY puntos rx m golpe anguloPelota tiempoLapso estado p (posBY!!ind) (posBY!!ind) modo index)
...
--     translate $ Vector3 (-px) (-py) (-(pz)::GLfloat)
--  preservingMatrix $ do
--     translate $ Vector3 (px) (py) ((pz+0.0001)::GLfloat)
...
temporizador pelotaX   pelotaY pelotaZ tiempo  ida   altura  ti fuerza preX xleta yleta puntos rx pendiente golpe anguloPelota tl estado pu xb yb modo index= do
       z <- get pelotaZ
       y <- get pelotaY 
       x <- get pelotaX     
       t <- get tiempo
...
-----------------------------CHOQUE CON BLANCO (SOLO PARA MODO 3)
                              else if ((z- 0.1)<= (-_RangoZ +0.01)  && y<= (by + _RADIOBlanco) &&  y>= (by- _RADIOBlanco) && x<= (bx + _RADIOBlanco) &&  x>= (bx - _RADIOBlanco) ) then do
